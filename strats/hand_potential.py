...
def potential(deck, hole_cards, community_cards, rank_data={}, look_ahead = 1): # deck has cards without the ones used, i.e. without community and hole cards
    deck = list(deck)
    ...
    w = {card: 1 for card in opp_cards} #dictionary 0 initialise
    hp = np.zeros((3, 3))
    hp_total = np.zeros(3)
    ...
    for opp_hole_cards in opp_cards:
        opp_hole_cards = list(opp_hole_cards)
        ...
        future_community_cards = list(combinations(future_community_cards, look_ahead)) # only implements one card look-ahead
        for future_cards in future_community_cards:
            future_cards = list(future_cards)

            hp_total[index] += w[tuple(opp_hole_cards)]
            ...
            if current_rank6 > opp_rank:
                hp[index][behind] += w[tuple(opp_hole_cards)]
            elif current_rank6 == opp_rank:
                hp[index][tied] += w[tuple(opp_hole_cards)]
            else:
                hp[index][ahead] += w[tuple(opp_hole_cards)]
